...
class lazy_mofo{
    ...
    public $grid_limit = 30;               // pagination limit number of records per page, verion >= 2018-07-10 set to 0 to disable pagination
    ...
    function get_output_control($column_name, $value, $control, $called_from){
        ...
        elseif($type == 'email')
        {
            return "<a href='javascript:enviar_mail($value)'>[enviar]</a>
            <script>
                function enviar_mail(id_paciente){
                    var opcion = confirm('¿Desea enviar por correo electronico el resultado?');
                    if (opcion == true) {
                        $.ajax({
                            url: 'send_mail.php',
                            type: 'post',
                            ...
                            success: function(text){
                                if (text == 'success'){                               
                                    window.location='index.php?enviado=1';
                                    formSuccess('Correo enviado satisfactoriamente');
                                } else {
                                    formError();
